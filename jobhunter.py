...
def show_stats():
    ...
    from scraper import print_stats
    init_db()
    print_stats()
...
async def main():
    ...
    if cmd == "scrape":
        ...
        from emailer import run_email_stage
        await run_email_stage()

    elif cmd == "guess-emails":
        from db import get_jobs, init_db
        from guesser import enrich_missing_emails
        init_db()
        ...
        try:
            subprocess.run([
                ...
                "api:app", "--reload", "--port", "8000", "--host", "0.0.0.0"
            ])
        ...
        from scheduler import run_pipeline_once
        ...
        from prospector import cmd_frenchtech
        city = sys.argv[2] if len(sys.argv) > 2 else "Bordeaux"
        await cmd_frenchtech(city)

    elif cmd == "score-prospects":
        from prospector import cmd_score
        await cmd_score()

    elif cmd == "prompts":
        from scraper import generate_prompts_only
